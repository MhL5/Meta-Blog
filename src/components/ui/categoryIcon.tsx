...
type CategoryIconProps = ComponentPropsWithoutRef<"span"> &
  VariantProps<typeof categoryIcon>;

const categoryIcon = cva("rounded-full border", {
  variants: {
    variant: {
      web_development:
...
/**
 * Use this to wrap `MetaBlogCategories` Icon
 * example:
 *     <CategoryIconWrapper variant={category} className="mr-3 p-1">
 *         <CategoryIcon className="h-4 w-4" />
 *     </CategoryIconWrapper>
 */
export default function CategoryIconWrapper({
  children,
  variant,
  className,
  ...props
  ...
  return (
    <span className={cn(categoryIcon({ variant }), className)} {...props}>
      {children}
    </span>
  );
}
